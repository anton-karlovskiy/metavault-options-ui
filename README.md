...
- `tailwindcss` & `@headlessui/react` & `@heroicons/react` & `@tailwindcss/forms`
- `storybook`
- `reach-ui`
- `react-table`
- `react-use`
...
- Upgrade `react-table` to v8 with built-in TypeScript.
- Remove `@ts-expect-error` from `src\components\Select` by properly fixing type issues.
